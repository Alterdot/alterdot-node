...
To develop alterdot-node:

```bash
cd ~
git clone git@github.com:<yourusername>/alterdot-node.git
...
We now will setup symlinks in `alterdot-node` *(repeat this for any other modules you're planning on developing)*:
```bash
cd node_modules
rm -rf bitcore-lib
ln -s ~/bitcore-lib
rm -rf bitcoind-rpc-alterdot
ln -s ~/bitcoind-rpc-alterdot
```
...
cd devnode
mkdir node_modules
touch alterdot-node.json
touch package.json
```

Edit `alterdot-node.json` with something similar to:
```json
{
  "network": "livenet",
  "port": 3001,
...
```bash
../alterdot-node/bin/alterdot-node start
```