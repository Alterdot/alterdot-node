...
function checkConfigVersion2(fullConfig) {
  ...
  if (!datadirUndefined || addressDefined || dbDefined) {
    ...
    console.warn('\nConfiguration file is not compatible with this version. \n' +
                 'A reindex for alterdotd is necessary for this upgrade with the "reindex=1" alterdot.conf option. \n' +
                 'There are changes necessary in both alterdot.conf and alterdot-node.json. \n\n' +
                 'To upgrade please see the details below and documentation at: \n' +
                 'https://github.com/Alterdot/alterdot-node/blob/alterdotd/docs/upgrade.md \n');
        ...
        servicesConfig: {
          alterdotd: {
            spawn: {
              ...
              exec: path.resolve(__dirname, '../../bin/alterdotd')
            }
          }
        }
      };
 ...
 * @param {Object} options.config - The parsed alterdot-node.json configuration file
...
function start(options) {
  ...
  fullConfig.path = path.resolve(options.path, './alterdot-node.json');
...
function loadModule(req, service) {
  try {
    // first try in the built-in alterdot-node services directory
    ...
    if (servicePackage.alterdotNode) {
      serviceModule = service.name + '/' + servicePackage.alterdotNode;
    }
 ...
 * @param {Node} node - The Alterdot Node instance
 */
