...
/**
 * Will remove a service from alterdot-node.json
...
/**
 * Will remove the Node.js service and from the alterdot-node configuration.
 * @param {String} options.cwd - The current working directory
 * @param {String} options.dirname - The alterdot-node configuration directory
...
function remove(options, done) {
  ...
  var alterdotConfigPath = path.resolve(configPath, 'alterdot-node.json');
  var packagePath = path.resolve(configPath, 'package.json');

  if (!fs.existsSync(alterdotConfigPath) || !fs.existsSync(packagePath)) {
    return done(
      new Error('Directory does not have a alterdot-node.json and/or package.json file.')
    );
  }
  ...
  async.eachSeries(
    ...
    function(service, next) {
      ...
      removeService(configPath, service, function(err) {
        ...
        // remove service to alterdot-node.json
        removeConfig(alterdotConfigPath, service, next);
      });
    }, done
  );
}
