...
var BASE_PACKAGE = {
  description: 'A full Alterdot node build with Alterdot',
  repository: 'https://github.com/user/project',
  license: 'MIT',
  readme: 'README.md',
  dependencies: {
    'alterdot-lib': '^' + alterdot.version,
    'alterdot-node': version
  }
};

/**
 * Will create a directory and alterdot.conf file for Alterdot.
 * @param {String} dataDir - The absolute path
 * @param {Function} done - The callback function called when finished
 */
function createAlterdotDirectory(datadir, done) {
...
/**
 * Will create a base Alterdot Node configuration directory and files.
 * @param {Object} options
 * @param {String} options.network - "testnet" or "livenet"
 * @param {String} options.datadir - The alterdot database directory
...
function createConfigDirectory(options, configDir, isGlobal, done) {
  mkdirp(configDir, function(err) {
    ...
    try {
      fs.writeFileSync(configDir + '/alterdot-node.json', configJSON);
      if (!isGlobal) {
...
/**
 * Will setup a directory with a Alterdot Node directory, configuration file,
 * alterdot configuration, and will install all necessary dependencies.
 *
 * @param {Object} options
 * @param {String} options.cwd - The current working directory
 * @param {String} options.dirname - The name of the alterdot node configuration directory
 * @param {String} options.datadir - The path to the alterdot datadir
...
function create(options, done) {
  ...
  async.series([
    function(next) {
      // Setup the the alterdot-node directory and configuration
    ...
    function(next) {
      // Setup the Alterdot directory and configuration
      if (!fs.existsSync(absDataDir)) {
        createAlterdotDirectory(absDataDir, next);
      } else {
        next();
      }
    },
