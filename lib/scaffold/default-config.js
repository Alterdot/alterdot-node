...
/**
 * Will return the path and default alterdot-node configuration. It will search for the
 * configuration file in the "~/.alterdot" directory, and if it doesn't exist, it will create one
...
function getDefaultConfig(options) {
  ...
  var defaultPath = path.resolve(process.env.HOME, './.alterdot');
  var defaultConfigFile = path.resolve(defaultPath, './alterdot-node.json');

  if (!fs.existsSync(defaultPath)) {
    mkdirp.sync(defaultPath);
  }

  var defaultServices = ['alterdotd', 'web'];
