...
var Proposal = alterdot.GovObject.Proposal;
...
/**
 * Provides a friendly event driven API to alterdotd in Node.js. Manages starting and
 * stopping alterdotd as a child process for application support, as well as connecting
 * to multiple alterdotd processes for server infrastructure. Results are cached in an
 * LRU cache for improved performance and methods added for common queries.
 *
 * @param {Object} options
 * @param {Node} options.node - A reference to the node
 */
function Alterdot(options) {
  if (!(this instanceof Alterdot)) {
    return new Alterdot(options);
  }
...
util.inherits(Alterdot, Service);

Alterdot.dependencies = [];

Alterdot.DEFAULT_MAX_TXIDS = 1000;
Alterdot.DEFAULT_MAX_HISTORY = 50;
Alterdot.DEFAULT_MAX_UTXO = 1000;
Alterdot.DEFAULT_SHUTDOWN_TIMEOUT = 15000;
Alterdot.DEFAULT_ZMQ_SUBSCRIBE_PROGRESS = 0.9999;
Alterdot.DEFAULT_MAX_ADDRESSES_QUERY = 10000;
Alterdot.DEFAULT_SPAWN_RESTART_TIME = 5000;
Alterdot.DEFAULT_SPAWN_STOP_TIME = 10000;
Alterdot.DEFAULT_TRY_ALL_INTERVAL = 1000;
Alterdot.DEFAULT_REINDEX_INTERVAL = 10000;
Alterdot.DEFAULT_START_RETRY_INTERVAL = 5000;
Alterdot.DEFAULT_TIP_UPDATE_INTERVAL = 15000;
Alterdot.DEFAULT_TRANSACTION_CONCURRENCY = 5;
Alterdot.DEFAULT_INSTANTSEND_FEE = 10000;
Alterdot.DEFAULT_CONFIG_SETTINGS = {
  server: 1,
  whitelist: '127.0.0.1',
  txindex: 1,
  addressindex: 1,
...
Alterdot.prototype._initDefaults = function(options) {
  /* jshint maxcomplexity: 15 */

  // limits
  this.maxTxids = options.maxTxids || Alterdot.DEFAULT_MAX_TXIDS;
  this.maxTransactionHistory = options.maxTransactionHistory || Alterdot.DEFAULT_MAX_HISTORY;
  this.maxUTXOHistory = options.maxUTXOHistory || Alterdot.DEFAULT_MAX_UTXO;
  this.maxAddressesQuery = options.maxAddressesQuery || Alterdot.DEFAULT_MAX_ADDRESSES_QUERY;
  this.shutdownTimeout = options.shutdownTimeout || Alterdot.DEFAULT_SHUTDOWN_TIMEOUT;

  // spawn restart setting
  this.spawnRestartTime = options.spawnRestartTime || Alterdot.DEFAULT_SPAWN_RESTART_TIME;
  this.spawnStopTime = options.spawnStopTime || Alterdot.DEFAULT_SPAWN_STOP_TIME;

  // try all interval
  this.tryAllInterval = options.tryAllInterval || Alterdot.DEFAULT_TRY_ALL_INTERVAL;
  this.startRetryInterval = options.startRetryInterval || Alterdot.DEFAULT_START_RETRY_INTERVAL;

  // rpc limits
  this.transactionConcurrency = options.transactionConcurrency || Alterdot.DEFAULT_TRANSACTION_CONCURRENCY;

  // sync progress level when zmq subscribes to events
  this.zmqSubscribeProgress = options.zmqSubscribeProgress || Alterdot.DEFAULT_ZMQ_SUBSCRIBE_PROGRESS;
};

Alterdot.prototype._initCaches = function() {
...
Alterdot.prototype._initClients = function() {
  var self = this;
  this.nodes = [];
  this.nodesIndex = 0;
 ...
 */
Alterdot.prototype.getAPIMethods = function() {
  var methods = [
    ...
    ['resolveDomain', this, this.resolveDomain, 1]
  ];
  return methods;
};
 ...
 */
Alterdot.prototype.getPublishEvents = function() {
  return [
    {
    ...
    {
      name: 'alterdotd/transactionlock',
      scope: this,
...
Alterdot.prototype.subscribe = function(name, emitter) {
  this.subscriptions[name].push(emitter);
  log.info(emitter.remoteAddress, 'subscribe:', 'alterdotd/' + name, 'total:', this.subscriptions[name].length);
};

Alterdot.prototype.unsubscribe = function(name, emitter) {
  var index = this.subscriptions[name].indexOf(emitter);
  if (index > -1) {
    this.subscriptions[name].splice(index, 1);
  }
  log.info(emitter.remoteAddress, 'unsubscribe:', 'alterdotd/' + name, 'total:', this.subscriptions[name].length);
};

Alterdot.prototype.subscribeAddress = function(emitter, addresses) {
  var self = this;
  ...
  if(addresses){
    for(var i = 0; i < addresses.length; i++) {
      if (alterdot.Address.isValid(addresses[i], this.node.network)) {
        addAddress(addresses[i]);
      }
    }
  }

  log.info(emitter.remoteAddress, 'subscribe:', 'alterdotd/addresstxid', 'total:', _.size(this.subscriptions.address));
};

Alterdot.prototype.unsubscribeAddress = function(emitter, addresses) {
  var self = this;
  if(!addresses) {
  ...
  log.info(emitter.remoteAddress, 'unsubscribe:', 'alterdotd/addresstxid', 'total:', _.size(this.subscriptions.address));
};
 ...
 */
Alterdot.prototype.unsubscribeAddressAll = function(emitter) {
...
Alterdot.prototype._getDefaultConfig = function() {
  var config = '';
  var defaults = Alterdot.DEFAULT_CONFIG_SETTINGS;
...
Alterdot.prototype._parseAlterdotConf = function(configPath) {
  var options = {};
...
Alterdot.prototype._expandRelativeDatadir = function() {
...
Alterdot.prototype._loadSpawnConfiguration = function(node) {
  /* jshint maxstatements: 25 */

  $.checkArgument(this.options.spawn, 'Please specify "spawn" in alterdotd config options');
  $.checkArgument(this.options.spawn.datadir, 'Please specify "spawn.datadir" in alterdotd config options');
  $.checkArgument(this.options.spawn.exec, 'Please specify "spawn.exec" in alterdotd config options');

  this._expandRelativeDatadir();

  var spawnOptions = this.options.spawn;
  var configPath = path.resolve(spawnOptions.datadir, './alterdot.conf');

  log.info('Using Alterdot config file:', configPath);
  ...
  _.extend(this.spawn.config, this._parseAlterdotConf(configPath));

  var networkConfigPath = this._getNetworkConfigPath();
  if (networkConfigPath && fs.existsSync(networkConfigPath)) {
    _.extend(this.spawn.config, this._parseAlterdotConf(networkConfigPath));
  }
...
Alterdot.prototype._checkConfigIndexes = function(spawnConfig, node) {
  $.checkState(
    spawnConfig.txindex && spawnConfig.txindex === 1,
    '"txindex" option is required in order to use transaction query features of alterdot-node. ' +
  ...
  $.checkState(
    ...
    '"addressindex" option is required in order to use address query features of alterdot-node. ' +
  ...
  $.checkState(
    ...
    '"spentindex" option is required in order to use spent info query features of alterdot-node. ' +
  ...
  $.checkState(
    ...
    '"zmqpubrawtx" and "zmqpubhashblock" are expected to the same host and port in alterdot.conf'
  );

  if (spawnConfig.reindex && spawnConfig.reindex === 1) {
    log.warn('Reindex option is currently enabled. This means that alterdotd is undergoing a reindex. ' +
             ...
             'of alterdot-node services will start.');
    node._reindex = true;
  }
};

Alterdot.prototype._resetCaches = function() {
 ...
 * @private
 */
Alterdot.prototype._tryAllClients = function(func, callback) {
  var self = this;
...
Alterdot.prototype._wrapRPCError = function(errObj) {
  var err = new errors.RPCError(errObj.message);
  err.code = errObj.code;
  return err;
};

Alterdot.prototype._initChain = function(callback) {
  var self = this;
    ...
    self.client.getBlock(response.result, function(err, response) {
      ...
      self.client.getBlockHash(0, function(err, response) {
        ...
        self.getRawBlock(blockhash, function(err, blockBuffer) {
          ...
          self.emit('ready');
          log.info('Alterdot Daemon Ready');
          callback();
        });
      });
...
Alterdot.prototype._getDefaultConf = function() {
  var networkOptions = {
    rpcport: 31050
  };
  if (this.node.network === alterdot.Networks.testnet) {
    networkOptions.rpcport = 31450;
  }
  return networkOptions;
};

Alterdot.prototype._getNetworkConfigPath = function() {
  var networkPath;
  if (this.node.network === alterdot.Networks.testnet) {
    networkPath = 'testnet3/alterdot.conf';
    if (this.node.network.regtestEnabled) {
      networkPath = 'regtest/alterdot.conf';
    }
  }
  return networkPath;
};

Alterdot.prototype._getNetworkOption = function() {
  var networkOption;
  if (this.node.network === alterdot.Networks.testnet) {
...
Alterdot.prototype._zmqBlockHandler = function(node, message) {
  var self = this;
  ...
  if (!self.zmqKnownBlocks.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.hashblock.length; i++) {
      this.subscriptions.hashblock[i].emit('alterdotd/hashblock', message.toString('hex'));
    }
  }

};

Alterdot.prototype._rapidProtectedUpdateTip = function(node, message) {
  var self = this;
...
Alterdot.prototype._updateTip = function(node, message) {
  var self = this;
    ...
    if(!self.node.stopping) {
      self.syncPercentage(function(err, percentage) {
        if (err) {
          ...
          log.info('Alterdot Height:', self.height, 'Percentage:', percentage.toFixed(2));
        }
      });
    }
  }
...
Alterdot.prototype._getAddressesFromTransaction = function(transaction) {
  var addresses = [];
...
Alterdot.prototype._notifyAddressTxidSubscribers = function(txid, transaction) {
  ...
  for (var i = 0; i < addresses.length; i++) {
    ...
    if(this.subscriptions.address[address]) {
      ...
      for(var j = 0; j < emitters.length; j++) {
        emitters[j].emit('alterdotd/addresstxid', {
          address: address,
          txid: txid
        });
      }
...
Alterdot.prototype._zmqTransactionHandler = function(node, message) {
  ...
  var self = this;
  var hash = alterdot.crypto.Hash.sha256sha256(message);
  ...
  if (!self.zmqKnownTransactions.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.rawtransaction.length; i++) {
      this.subscriptions.rawtransaction[i].emit('alterdotd/rawtransaction', message.toString('hex'));
    }

    var tx = alterdot.Transaction();
    tx.fromString(message);
    var txid = alterdot.util.buffer.reverse(hash).toString('hex');
    self._notifyAddressTxidSubscribers(txid, tx);

  }
};

Alterdot.prototype._zmqTransactionLockHandler = function (node, message) {
  ...
  if (!self.zmqKnownTransactionLocks.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.transactionlock.length; i++) {
      this.subscriptions.transactionlock[i].emit('alterdotd/transactionlock', message.toString('hex'));
    }

  }
};

Alterdot.prototype._checkSyncedAndSubscribeZmqEvents = function(node) {
  var self = this;
  var interval;
  ...
  checkAndSubscribe(function(err, synced) {
    ...
    if (!synced) {
      ...
      }, node._tipUpdateInterval || Alterdot.DEFAULT_TIP_UPDATE_INTERVAL);
    }
  });

};

Alterdot.prototype._subscribeZmqEvents = function(node) {
  var self = this;
...
Alterdot.prototype._initZmqSubSocket = function(node, zmqUrl) {
  var self = this;
...
Alterdot.prototype._checkReindex = function(node, callback) {
  var self = this;
  var interval;
  function finish(err) {
    clearInterval(interval);
  ...
  interval = setInterval(function() {
    node.client.getBlockchainInfo(function(err, response) {
      ...
      log.info('Alterdot Daemon Reindex Percentage: ' + percentSynced.toFixed(2));
  ...
  }, node._reindexWait || Alterdot.DEFAULT_REINDEX_INTERVAL);

};

Alterdot.prototype._loadTipFromNode = function(node, callback) {
  var self = this;
  node.client.getBestBlockHash(function(err, response) {
    if (err && err.code === -28) {
...
Alterdot.prototype._stopSpawnedAlterdot = function(callback) {
  var self = this;
  var spawnOptions = this.options.spawn;
  var pidPath = spawnOptions.datadir + '/alterdotd.pid';
  ...
  function stopProcess() {
    fs.readFile(pidPath, 'utf8', function(err, pid) {
      ...
      try {
        log.warn('Stopping existing spawned alterdot process with pid: ' + pid);
        self._process.kill(pid, 'SIGINT');
      } catch(err) {
        if (err && err.code === 'ESRCH') {
          log.warn('Unclean alterdot process shutdown, process not found with pid: ' + pid);
          return callback(null);
        } else if(err) {
          return callback(err);
        }
...
Alterdot.prototype._spawnChildProcess = function(callback) {
  var self = this;
  ...
  self._stopSpawnedAlterdot(function(err) {
    if (err) {
      return callback(err);
    }

    log.info('Starting alterdot process');
    ...
    self.spawn.process.once('exit', function(code) {
      if (!self.node.stopping) {
        log.warn('Alterdot process unexpectedly exited with code:', code);
        log.warn('Restarting alterdot child process in ' + self.spawnRestartTime + 'ms');
        setTimeout(function() {
          self._spawnChildProcess(function(err) {
            if (err) {
              return self.emit('error', err);
            }
            log.warn('Alterdot process restarted');
          });
    ...
    async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
      ...
      if (exitShutdown) {
        return callback(new Error('Stopping while trying to spawn alterdotd.'));
      }
...
Alterdot.prototype._connectProcess = function(config, callback) {
  var self = this;
  var node = {};
  var exitShutdown = false;
  ...
  async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
    ...
    if (exitShutdown) {
      return callback(new Error('Stopping while trying to connect to alterdotd.'));
    }
 ...
 */
Alterdot.prototype.start = function(callback) {
  var self = this;
  ...
  async.series([
    ...
    if (self.nodes.length === 0) {
      return callback(new Error('Alterdot configuration options "spawn" or "connect" are expected'));
    }
 ...
 * @param {Function} callback
 */
Alterdot.prototype.isSynced = function(callback) {
 ...
 * @param {Function} callback
 */
Alterdot.prototype.syncPercentage = function(callback) {
  var self = this;
...
Alterdot.prototype._normalizeAddressArg = function(addressArg) {
 ...
 * @param {String|Address|Array} addressArg - An address string, alterdot address, or array of addresses
 * @param {Object} options
 * @param {Function} callback
 */
Alterdot.prototype.getAddressBalance = function(addressArg, options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Alterdot.prototype.getAddressUnspentOutputsPaginated = function(addressArg, options, callback) {
  var self = this;
  ...
  function transformUnspentOutput(delta) {
    var script = alterdot.Script.fromAddress(delta.address);
    return {
 ...
 * @param {Function} callback
 */
Alterdot.prototype.getAddressUnspentOutputs = function(addressArg, options, callback) {
  var self = this;
  var queryMempool = _.isUndefined(options.queryMempool) ? true : options.queryMempool;
  var addresses = self._normalizeAddressArg(addressArg);
  var cacheKey = addresses.join('');
...
Alterdot.prototype._getBalanceFromMempool = function(deltas) {
  var satoshis = 0;
...
Alterdot.prototype._getTxidsFromMempool = function(deltas) {
  var mempoolTxids = [];
...
Alterdot.prototype._getHeightRangeQuery = function(options, clone) {
  if (options.start >= 0 && options.end >= 0) {
    if (options.end > options.start) {
      throw new TypeError('"end" is expected to be less than or equal to "start"');
    }
    if (clone) {
      // reverse start and end as the order in alterdot is most recent to less recent
 ...
 * @param {Function} callback
 */
Alterdot.prototype.getAddressTxids = function(addressArg, options, callback) {
...
Alterdot.prototype._getConfirmationsDetail = function(transaction) {
...
Alterdot.prototype._getAddressDetailsForInput = function(input, inputIndex, result, addressStrings) {
  if (!input.address) {
    return;
  }
...
Alterdot.prototype._getAddressDetailsForOutput = function(output, outputIndex, result, addressStrings) {
  if (!output.address) {
    return;
  }
...
Alterdot.prototype._getAddressDetailsForTransaction = function(transaction, addressStrings) {
  var result = {
    addresses: {},
    satoshis: 0
  };
 ...
 * @param {Object} txid - A alterdot transaction id
 * @param {Function} callback
 */
Alterdot.prototype._getAddressDetailedTransaction = function(txid, options, next) {
  var self = this;
...
Alterdot.prototype._getAddressStrings = function(addresses) {
  var addressStrings = [];
  for (var i = 0; i < addresses.length; i++) {
    var address = addresses[i];
    if (address instanceof alterdot.Address) {
...
Alterdot.prototype._paginate = function(fullArray, fromArg, toArg, fromHeight, toHeight) {
  var slicedArray;
  var filteredArray;
 ...
 * @param {Function} callback
 */
Alterdot.prototype.getAddressHistory = function(addressArg, options, callback) {
  var self = this;
  var addresses = self._normalizeAddressArg(addressArg);
 ...
 * @param {Function} callback
 */
Alterdot.prototype.getAddressSummary = function(addressArg, options, callback) {
  var self = this;
  var summary = {};
  var queryMempool = _.isUndefined(options.queryMempool) ? true : options.queryMempool;
  var summaryTxids = [];
 ...
 * @private
 */
Alterdot.prototype._maybeGetBlockHash = function(blockArg, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Alterdot.prototype.getRawBlock = function(blockArg, callback) {
 ...
 * @param {Function} callback
 */
Alterdot.prototype.getBlockOverview = function(blockArg, callback) {
  var self = this;
...
/**
 * Will retrieve a block as a Alterdot object
 * @param {String|Number} block - A block hash or block height number
 * @param {Function} callback
 */
Alterdot.prototype.getBlock = function(blockArg, callback) {
  // TODO apply performance patch to the RPC method for raw data
  var self = this;
    ...
    if (cachedBlock) {
      ...
      self._tryAllClients(function(client, done) {
        client.getBlock(blockhash, false, function(err, response) {
          ...
          var blockObj = alterdot.Block.fromString(response.result);
 ...
 * @param {Function} callback
 */
Alterdot.prototype.getBlockHashesByTimestamp = function(high, low, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Alterdot.prototype.getBlockHeader = function(blockArg, callback) {
  var self = this;
 ...
 */
Alterdot.prototype.getBlockHeaders = function(blockArg, callback, nbOfBlockToFetch) {
	var self = this;
	var _toFetch = 25;
 ...
 * @param {Function} callback
 */
Alterdot.prototype.estimateFee = function(blocks, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Alterdot.prototype.sendTransaction = function(tx, options, callback) {
  var self = this;
...
Alterdot.prototype._getRawTransactionData = function(txid, callback) {
  var self = this;
  self._tryAllClients(function(client, done) {
 ...
 * @param {Function} callback
 */
Alterdot.prototype.getRawTransaction = function(txid, callback) {
  var self = this;
...
/**
 * Will get a transaction as a Alterdot Transaction. Results include the mempool.
 * @param {String} txid - The transaction hash
 * @param {Boolean} queryMempool - Include the mempool
 * @param {Function} callback
 */
Alterdot.prototype.getTransaction = function(txid, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Alterdot.prototype.getDetailedTransaction = function(txid, callback) {
  var self = this;
 ...
 * @param callback
 */
Alterdot.prototype.govObjectList = function(options, callback) {
  var self = this;
...
Alterdot.prototype.getCurrentVotes = function (govhash, callback) {
  var self = this;
...
Alterdot.prototype.getVotes = function (govhash, callback) {
  var self = this;
...
Alterdot.prototype.getSuperBlockBudget = function (blockindex, callback) {
  var self = this;
...
/*jshint maxparams: 6 */
Alterdot.prototype.govObjectSubmit = function (parentHash, revision, time, dataHex, feeTxId, callback) {
  var self = this;
...
Alterdot.prototype.govObjectDeserialize = function (hexdata, callback) {
  var self = this;
...
Alterdot.prototype.govObjectCheck = function(hexdata, callback){
  var self = this;
...
Alterdot.prototype.govObjectInfo = function (callback) {
  var self = this;
...
Alterdot.prototype.govCount = function (callback) {
  var self = this;
...
Alterdot.prototype.getSpork = function(callback){
	var self = this;
	var SporksList = {};
...
Alterdot.prototype.getMNList = function(callback){
  var self = this;
  var rawResults= {};
  var MNList = [];
 ...
 * @param callback
 */
Alterdot.prototype.govObjectHash = function(hash, callback) {
  var self = this;
 ...
 * @param callback
 */
Alterdot.prototype.govObjectCheck = function govObjectCheck(hexdata, callback) {
  var self = this;
  this.client.gobject('check', hexdata, function (err, response) {
    if (err) {
      return callback(self._wrapRPCError(err));
 ...
 * @param {Function} callback
 */
Alterdot.prototype.getBestBlockHash = function(callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Alterdot.prototype.getBestChainLock = function (callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Alterdot.prototype.getSpentInfo = function(options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Alterdot.prototype.getInfo = function(callback) {
  var self = this;
...
Alterdot.prototype.generateBlock = function(num, callback) {
  var self = this;
...
/**
 * Will resolve a BDNS domain.
 * @param {Function} callback
 */
Alterdot.prototype.resolveDomain = function(domainName, callback) {
  var self = this;
  this.client.resolveDomain(domainName, function(err, response) {
    if (err) {
      return callback(self._wrapRPCError(err));
    }
    callback(null, response.result);
 ...
 * @param {Function} callback
 */
Alterdot.prototype.stop = function(callback) {
  if (this.spawn && this.spawn.process) {
    ...
    this.spawn.process.once('exit', function(code) {
      if (!exited) {
        exited = true;
        if (code !== 0) {
          var error = new Error('alterdotd spawned process exited with status code: ' + code);
          error.code = code;
    ...
    setTimeout(function() {
      if (!exited) {
        exited = true;
        return callback(new Error('alterdotd process did not exit'));
      }
